...
- **Failure notice latency.** Run `sim.mjs test <run id> FailNoticeLatencyProbe`. It times the Send tap for `fixture:fail 500` to `chat.turn.notice` on screen and attaches the milliseconds as `fail-notice-latency-ms`; the time includes the 1.5 second window.
- **Hang and watchdog.** Send `fixture:hang`. Run `sim.mjs test <run id> HangWatchdogProof`. `chat.working` shows for 30 seconds after the 1.5 second window, then `chat.turn.notice` reads `The model provider is having trouble — try again in a few minutes.` with `Try again`, the working row is gone, and `chat.error` does not exist. Attachments `hang-working` and `hang-watchdog` show both states, and `hang-watchdog-records` shows `turnSettled 1`. Interactively, send `fixture:hang`, wait 35 seconds, and capture with `sim.mjs shot <run id> hang-watchdog`.
