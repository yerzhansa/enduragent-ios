...
- `chat-failed` shows one catalog sentence in `chat.turn.notice` under the message when the model fails, chosen by the failure: `The model provider is having trouble — try again in a few minutes.` with `Try again` in `chat.turn.tryAgain` for a server error, a network failure, a timeout, or the watchdog; `The model provider rejected the API key — check your provider credentials.` with no `Try again` for a rejected key; `Rate limited — please try again in ~7 seconds.` with `Try again` for a rate limit with a 7 second hint; `Sorry, something went wrong. Please try again.` for exhausted credits (no `Try again`), an overflow, or a bad request; and `The coach couldn't respond. Please try again.` for a broken or unknown stream. No status code, server body, or Swift type name is ever on screen. `chat.error` no longer exists for turns.
...
- `chat-saved-unverified` shows `Sorry, something went wrong. Please try again.` in `chat.turn.notice` with no `Try again` when coach memory was saved before the reply failed. The sentence is a placeholder until the approved notice lands. Records lists the `memorySection` row stamped with the turn's attempt.
...
- **Saved, then failed.** This step is interactive. Send `fixture:memory-then-fail`. `chat.turn.notice` reads `Sorry, something went wrong. Please try again.` and `chat.turn.tryAgain` does not exist. Menu, Debug, Records lists `memorySection 1`. Capture it with `sim.mjs shot <run id> saved-unverified`.
