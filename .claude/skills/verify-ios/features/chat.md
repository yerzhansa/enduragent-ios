...
- `chat-accepted-relaunch` shows a message that was sent but never started before the app closed once, with `Received before the app closed. Tap Try again to send it.` in `chat.turn.receivedBeforeClose` and `Try again`; nothing runs until the tap. A message queued behind a running reply when the app was killed reopens this way.
- `chat-interrupted-relaunch` shows a message whose reply had started when the app was killed with `This reply stopped before it finished. Nothing was changed.` in `chat.turn.notice` and `Try again`, never `Received before the app closed`. When coach memory was saved before the kill, the notice reads `This reply stopped before it finished. Some information was saved first.` with no `Try again`. No reply text shows, and relaunch starts no model call on its own. Records lists the turn's `turnSettled` row as `interrupted processEnded`.
- `chat-background` keeps a reply running when the app goes to the background and comes back; backgrounding alone never shows an interrupted notice. A message still inside its 1.5 second join window starts at once when the app goes to the background.
...
- **Saved, then failed.** Send `fixture:memory-then-fail`. `NoticeCopyProof` covers it. `chat.turn.notice` reads `I saved your information, but couldn't verify my response. Please try again.` and `chat.turn.tryAgain` does not exist. Interactively, Menu, Debug, Records lists `memorySection 1`. Capture it with `sim.mjs shot <run id> saved-unverified`.
...
- **Failure copy.** Send `fixture:fail 401`, then `fixture:fail network x3`, then `fixture:fail 429 7 x4`. Run `sim.mjs test <run id> FailureCopyProof`. Attachment `failure-copy-credentials` shows `Your Credits couldn't be used. Restore purchases to continue.` with `Restore purchases` and no `Try again`, `failure-copy-network` shows `The model provider is having trouble — try again in a few minutes.` with `Try again`, and `failure-copy-rate-limited` shows `Rate limited — please try again in ~7 seconds.` with `Try again`. `fixture:fail timeout x2` shows the provider-down sentence, and `fixture:fail overflow x4` shows `Sorry, something went wrong. Please try again.`; interactively, send each and capture it with `sim.mjs shot`.
- **Notice copy.** Send `fixture:fail 402`, tap `chat.turn.buyCredits`, go back, then send `fixture:fail 401` and `fixture:memory-then-fail`. Run `sim.mjs test <run id> NoticeCopyProof`. Attachments `notice-copy-credits-exhausted`, `notice-copy-buy-credits-opens-credits`, `notice-copy-access-rejected`, and `notice-copy-saved-unverified` show each sentence, the Credits screen that `Buy Credits` opens, and no `Try again` on any of the three.
- **Access notices.** Launch with `-EnduragentFixtureKeychain empty` for a fixture with no Credits key, or `-EnduragentFixtureKeychain locked` for a locked keychain, finish onboarding, and send a message. Run `sim.mjs test <run id> AccessNoticeProof`. Attachment `access-not-configured` shows `Choose how the coach reaches a model to continue.` with `Choose access method`, `access-not-configured-connect` shows the connect step it opens, and `access-locked` shows `Unlock your iPhone to continue. Your message is saved.` with `Try again`.
- **Hang and watchdog.** This step is interactive. Send `fixture:hang` and wait 65 seconds. `chat.working` shows for 60 seconds, because the first watchdog timeout gets one plain retry that hangs again, then `chat.turn.notice` reads `The model provider is having trouble — try again in a few minutes.` with `Try again`. Capture it with `sim.mjs shot <run id> hang-watchdog`.
- **Kill while replying, reopen, Try again.** Send `fixture:hang`, wait for `turnClaim 1` in Records, terminate the app, and relaunch with the kept store. Run `sim.mjs test <run id> InterruptedAfterKillProof`. Attachment `interrupted-after-kill` shows the message once with `This reply stopped before it finished. Nothing was changed.` and `Try again` and no reply text, `interrupted-after-kill-records` lists `userMessage 1`, `turnClaim 1`, and `turnSettled 1`, and `interrupted-after-kill-try-again` shows the week summary after the tap. The last Records read lists `turnClaim 2`, `turnSettled 2`, and the first settlement as `interrupted processEnded`. Interactively, send `fixture:hang`, wait about two seconds, run `sim.mjs launch <run id> --keep`, and tap `chat.turn.tryAgain`.
- **Saved, then killed.** Send `fixture:memory-then-hang`, wait for `memorySection 1` in Records, terminate, and relaunch with the kept store. Run `sim.mjs test <run id> SavedWorkInterruptedProof`. Attachment `saved-work-interrupted` shows `This reply stopped before it finished. Some information was saved first.` with no `Try again`, and `saved-work-interrupted-records` lists `memorySection 1` and `turnSettled 1`.
- **Queued message, kill, reopen.** Send `fixture:hang`, then the week question while the first reply hangs, terminate, and relaunch with the kept store. Run `sim.mjs test <run id> QueuedTurnAfterKillProof`. Attachment `queued-turn-after-kill` shows the first message with the nothing-changed notice and the second with `Received before the app closed. Tap Try again to send it.`, each with `Try again`. Tapping the second `Try again` answers it with the week summary; Records lists `userMessage 2`, `turnClaim 2`, and `turnSettled 2`.
- **Background and back.** Send `fixture:slow`, press Home, wait five seconds, and reopen the app. Run `sim.mjs test <run id> BackgroundResumeProof`. Attachment `background-resume` shows the whole week summary and no notice.
...
- `app.terminate()` in a proof and `sim.mjs launch --keep` kill the app without `willTerminate`, so a reply cut off that way reopens as `interrupted processEnded` with no partial text. Only a real `willTerminate` keeps the partial text and records `interrupted appTerminating`; the hosted test `willTerminateSettlesTheRunningTurnBeforeItReturns` posts that notification.
- A kill within 1.5 seconds of Send lands before the reply starts, so the message reopens as `Received before the app closed` instead of the interrupted notice. Wait for `turnClaim 1` in Records before the kill.
...
- The fixture calendar is fixed on 1998-06-15, so a rate limit's wait has always ended by the time the notice shows, and its `Try again` is enabled at once. A fixture run cannot show the disabled wait.
