	...
	@Suite struct EnvelopeMigrationTests {
		...
		@Test func v1RowsDecodeAsLegacyBodies() async throws {
			...
			#expect(
				synced.records[2].body
					== .legacy(
						.userMessageV1(chatId: .main, athleteText: "/review", slash: .review)))
			#expect(
		...
		@Test func v1TranscriptFoldsWithLegacyRepliesSettled() async throws {
			...
			let conversation = ConversationFold.fold(
				chat: .main, synced: page.records, device: phoneA)
			...
			try await seed(
				...
				[
					...
					storedRecord(
						...
						body: .legacy(
							.windowStartV1(chatId: .main, firstIncludedUlid: fixedUlid(3)))),
					storedRecord(
		...
		@Test func upgradedStoreAcceptsEnvelopeV2Writes() async throws {
			...
			let conversation = ConversationFold.fold(
				chat: .main, synced: page.records, device: phoneA)
			#expect(
				conversation.current.messages.map(\.text).suffix(2) == [
					"after the upgrade", "Noted.",
				])
		...
		@Test func unknownKindBecomesSkippedRowNotAThrow() async throws {
			...
			let log = try store.open(deviceId: phoneA)
			try store.insertRaw(
				kind: "hologram", bodyVersion: 2, ulid: "01HGRAM0000000000000000000")
			let page = try await log.fetch(RecordQuery(scope: .everySynced))
			#expect(
				page.skipped == [.newerKind(kind: "hologram", ulid: "01HGRAM0000000000000000000")])
		...
		@Test func newerBodyVersionBecomesSkippedRow() async throws {
			...
			#expect(
				page.skipped == [
					.newerVersion(
						kind: "userMessage", version: 99, ulid: "01NEWVERS00000000000000000")
				])
